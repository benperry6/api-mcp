...
function unpaidOwnerShipmentId(state: ExactOrderState): string {
...
async function waitForUnpaidOwnerShipment(
  orderId: string,
  attempts = 10,
  delayMs = 1000
): Promise<ExactOrderState> {
  let lastState: ExactOrderState | undefined;
...
async function resolveShipmentId(
  ...
  const rawShipmentId = typeof confirmData.shipmentsId === 'string'
    ...
    : '';
  const explicitFailure = confirmData.submitSuccess === false
  ...
  try {
    switch (name) {
      ...
      case 'submit_order_to_cart': {
        ...
        let sotcShipmentsId = sotcInitialState.status === 'UNPAID'
          ? unpaidOwnerShipmentId(sotcInitialState)
          : sotcInitialState.childCode;
